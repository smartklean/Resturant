...
@section('content')
    ...
    <div class="col-md-8">
        ...
        <form class="navbar-form pull-center" action="{{ route('product') }}" method="get">
            @csrf
        ...
        <form class="navbar-form pull-center" action="{{ route('product.new') }}" method="get">
            @csrf

            <button type="submit" class="btn btn-success pull-right" >New Product <i class="fa fa-plus"></i></button>
        </form>
        <br><br>
        <div class="mytable">
                    ...
                    <tbody>
                        ...
                        @forelse($products as $index => $pass)
                            <tr>
                                ...
                                <td>{{$pass->quantity}}</td>
                                 <td>{{$pass->selling_price}}</td>
                                <td>{{$pass->created_at}}</td>
                                <td colspan="2">
                                    <a class="btn btn-primary fa fa-pencil" href="{{ route('product.edit', $pass->id) }}"></a> 
                                    &nbsp;¦¦&nbsp;
